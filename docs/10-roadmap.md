# 10 - نقشه راه (Roadmap)

این سند مراحل توسعه ادیتور Penman را مشخص می‌کند.

## مایلستون 1: پی‌ریزی (Foundation) - [وضعیت: کامل]
- [x] راه‌اندازی ساختار پروژه و بیلد سیستم (Vite/Rollup).
- [x] پیاده‌سازی Core Engine و مدیریت چرخه حیات.
- [x] پیاده‌سازی همگام‌سازی پایه با `<textarea>`.
- [x] طراحی و پیاده‌سازی معماری Event Emitter.

## مایلستون 2: هسته ویرایش (Editing Core)
- [x] پیاده‌سازی Selection System پایه.
- [x] پیاده‌سازی سیستم Undo/Redo با قابلیت Throttling.
- [x] پیاده‌سازی Command System (ابتدا بر پایه `execCommand`).

## مایلستون 3: رابط کاربری (UI & Toolbar)
- [x] ایجاد سیستم رندر Toolbar.
- [x] طراحی مجموعه اولیه آیکون‌های SVG.
- [x] پیاده‌سازی به‌روزرسانی وضعیت دکمه‌ها (Active state) بر اساس مکان نشانگر.

## مایلستون 4: پلاگین‌های پایه (Basic Plugins) - [وضعیت: نیمه‌کاره]
- [x] پلاگین فرمت‌های پایه (Bold, Italic, Underline).
- [x] پلاگین ایجاد لیست (UL, OL).
- [x] پلاگین درج لینک با دیالوگ ساده.

## مایلستون 5: پیشرفته و امنیت - [وضعیت: نیمه‌کاره]
- [ ] جایگزینی تدریجی `execCommand` با دستکاری DOM اختصاصی. (ثبت شده به عنوان بدهی فنی برای فازهای بعدی)
- [x] پیاده‌سازی ماژول Sanitization برای جلوگیری از XSS.
- [x] مدیریت رویداد Paste و تمیز کردن محتوای وارد شده.

## مایلستون 6: رابط کاربری و استایل‌ها (UI & Style) - [وضعیت: کامل]
- [x] پیاده‌سازی کامپوننت Base Dropdown.
- [x] پیاده‌سازی پلاگین انتخاب نوع بلوک (Block Type Selector).
- [x] استخراج CSSها به فایل‌های مجزا و بهبود استایل‌های عمومی ادیتور (طراحی TinyMCE-like).

## مایلستون 7: انتشار و مستندسازی - [وضعیت: نیمه‌کاره]
- [x] نوشتن تست‌های Unit.
- [ ] تکمیل مستندات کاربری و API.
- [ ] انتشار نسخه 1.0 (تثبیت API).
