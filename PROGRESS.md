...
- **وضعیت:** نیمه‌کاره (In Progress)
- **درصد پیشرفت:** 90%
- **توضیحات:** پوشش تست‌های واحد کامل شد. زیرساخت تست E2E احیا شد و تست‌های اصلی (فرمت، لینک، لیست، جداول، و عکس) نوشته شده‌اند. با این حال استفاده از Mock برای توابع اصلی مرورگر مانند `execCommand` ناقض قانون ۱۱ (عدم استفاده از Mock در سناریوهای اصلی) می‌باشد و این بدهی باید رفع گردد.
- **بخش ثبت تغییرات (Change Log):**
...
- **لیست اهداف:**
  ...
  - [ ] انتشار نسخه نهایی
- **بخش ثبت تغییرات (Change Log):**
  - **[$(date -u +%Y-%m-%dT%H:%M:%SZ)]**: پوشه مستندات پلاگین‌ها با پیروی از قانون تک‌منبع (Single Source of Truth) به طور کامل به پوشه `docs/` منتقل شد و وضعیت مستندسازی تثبیت گردید.
...
- **بخش ثبت تغییرات (Change Log):**
  ...
  - **[2026-04-17T07:29:18Z]**: اصلاح اساسی و تثبیت تمام پلاگین‌ها به E2E واقعی. سیستم Mock در تست‌های Integration (Image/Table/FindReplace) با اسکریپت Playwright جایگزین شد. همچنین Trust Level مربوط به ImagePlugin اصلاح شد تا پلاگین‌ها با ۱۰۰٪ استانداردهای معماری در وضعیت کامل قرار گیرند.
  - **[2026-04-17]**: بازیابی کامل زیرساخت تست E2E، حذف کدهای UI نمایشی و گسترش پوشش تست‌ها برای پلاگین‌های پایه. به‌روزرسانی مستندات جهت انعکاس وضعیت واقعی پروژه (تغییر وضعیت فازهای 4، 5، و 7 به نیمه‌کاره).
