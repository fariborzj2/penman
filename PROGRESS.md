...
- **بخش ثبت تغییرات (Change Log):**
  ...
  - **[2026-04-10T10:50:00Z]**: پس از بررسی دقیق بر اساس شواهد و فایل `docs/03-core-engine.md`، مشخص شد که فاز 1 نقص دارد. ساختار DOM دقیق نیست، Toolbar و Statusbar ساخته نمی‌شوند، متدهای عمومی `getContent()`, `setContent()`, `focus()`, `destroy()` وجود ندارند، و سیستم Instance Management (`penman.get`) پیاده‌سازی نشده است. وضعیت به "نیمه‌کاره" و پیشرفت به 80% برگردانده شد.
  - **[2026-04-10T10:55:00Z]**: سیستم Instance Management پیاده‌سازی شد. ساختار DOM و کلاس‌ها اصلاح شد. متدهای API شامل `getContent()`, `setContent()`, `focus()`, و `destroy()` پیاده‌سازی و تست شدند. وضعیت فاز 1 به "کامل" (100%) ارتقاء یافت و آماده مرج است.
...
- **درصد پیشرفت:** 0%
- **لیست اهداف:**
  - [ ] طراحی معماری و API برای Selection System پایه.
  - [ ] طراحی ساختار داده و منطق سیستم Undo/Redo (History Stack).
  - [ ] طراحی معماری Command System (رویکرد هیبریدی `execCommand` و دستکاری DOM).
- **بخش ثبت تغییرات (Change Log):**
